...
student *findstudent(vector<student> &allstudents,int key) {
	for (unsigned int i = 0; i < allstudents.size(); i++) if (allstudents[i].id  == key) return &allstudents[i];
	return nullptr;
}
...
int main(){
	...
	while (getline(course_file,textline)) {
		if(state == 1) {
			...
			if (textline == "> Students") state = 3;
			else allcourses.back().lecture_list.push_back(textline);   
		}
		else {
			if (textline == "---------------------------------------") state = 1;
			else {
